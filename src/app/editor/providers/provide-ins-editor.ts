...
const EXTENSIONS = [
  ...
  },
  {
    key: 'trailingNode',
    default: true,
    async loader(options: Partial<TrailingNodeOptions>) {
      const { TrailingNode } = await import('@tiptap/extensions');
      return TrailingNode.configure(options);
    },
  },
  {
    key: 'heading',
  ...
  //         const {InsCustomEnter} = await import('../extensions/enter');
  ...
  },
  {
    ...
    default: true,
    async loader(options: Partial<Record<string, unknown>>) {
      const { InsJumpAnchor } = await import('../extensions/jump-anchor');
  ...
  },
  {
    ...
    default: true,
    async loader(options: Partial<Record<string, unknown>>) {
      const { InsTabExtension } = await import('../extensions/indent-outdent');
...
export function provideInsEditor<O, S>(
  ...
  return {
    ...
    useFactory(): ReadonlyArray<AsyncExtension<O, S>> {
      ...
      const ownExtensions =
        typeof overrides === 'function' ? extensions.concat(overrides) : extensions;
