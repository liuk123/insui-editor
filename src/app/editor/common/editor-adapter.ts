...
import {
  ...
  isEditorSelectionContextEqual,
  type InsEditorSelectionContext,
  type InsEditorTableSelectionContext,
} from './selection-context';

export { ActiveNodePath } from './selection-context';
export type {
  Attrs,
  InsEditorSelectionContext,
  InsEditorTableSelectionContext,
} from './selection-context';
...
export abstract class AbstractInsEditor {
  ...
  public readonly selectionContext$: Observable<InsEditorSelectionContext> = this.transaction$.pipe(
    startWith(undefined),
    debounceTime(10),
    map(() => buildEditorSelectionContext(this.state)),
    distinctUntilChanged(isEditorSelectionContextEqual),
    shareReplay({ bufferSize: 1, refCount: true }),
  );
  public readonly activeNodeChange$: Observable<ActiveNodePath | null> =
    this.selectionContext$.pipe(
      map((context) => context.activeNode),
      distinctUntilChanged((a, b) => a?.node === b?.node && a?.nodePos === b?.nodePos),
      shareReplay({ bufferSize: 1, refCount: true }),
    );
  public readonly activeBlockChange$: Observable<ActiveNodePath | null> =
    this.selectionContext$.pipe(
      map((context) => context.activeBlock),
      distinctUntilChanged((a, b) => a?.node === b?.node && a?.nodePos === b?.nodePos),
      shareReplay({ bufferSize: 1, refCount: true }),
    );
  public readonly tableSelectionContext$: Observable<InsEditorTableSelectionContext | null> =
    this.selectionContext$.pipe(
  ...
  public abstract getSelectionContext(
    maxChars?: number,
  ...
  public abstract setFigure(options: {
    src: string;
    alt?: string;
    title?: string;
    caption?: string;
