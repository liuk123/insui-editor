    ...
    return {
      open: {
        ...
        renderHTML: (attributes) => ({
          open: attributes['open'] && this.options.inheritOpen ? 'open' : undefined,
        }),
      },
    };
  },
    ...
    return ({ node, getPos, editor }): any => {
      if (globalThis.document) {
        ...
        const openHandler = (event: Event): void => {
          ...
          (node.attrs as unknown as Record<string, unknown>)['open'] = details.open;

          event.target?.dispatchEvent(new CustomEvent(INS_EDITOR_RESIZE_EVENT, { bubbles: true }));
